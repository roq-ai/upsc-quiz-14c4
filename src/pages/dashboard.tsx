...
import { getTestSeries } from 'apiSdk/test-series';
import { TestSeriesInterface } from 'interfaces/test-series';
import { getSubjectiveQuestions } from 'apiSdk/subjective-questions';
import { SubjectiveQuestionInterface } from 'interfaces/subjective-question';
import { getCurrentAffairs } from 'apiSdk/current-affairs';
import { CurrentAffairsInterface } from 'interfaces/current-affairs';

import { SubjectiveQuestionListPage as ShowCaseList } from 'pages/subjective-questions';

function DashboardPage() {
  const { session } = useSession();

  const testSeriesStat = useSWR<PaginatedInterface<TestSeriesInterface>>(
    () => `/test-series`,
    () => getTestSeries({ limit: 1, offset: 0 }),
  );

  const subjectiveQuestionStat = useSWR<PaginatedInterface<SubjectiveQuestionInterface>>(
    () => `/subjective-questions`,
    () => getSubjectiveQuestions({ limit: 1, offset: 0 }),
  );

  const currentAffairsStat = useSWR<PaginatedInterface<CurrentAffairsInterface>>(
    () => `/current-affairs`,
    () => getCurrentAffairs({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Test Series',
      result: testSeriesStat?.data?.totalCount,
      isLoading: testSeriesStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Subjective Questions',
      result: subjectiveQuestionStat?.data?.totalCount,
      isLoading: subjectiveQuestionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Current Affairs',
      result: currentAffairsStat?.data?.totalCount,
      isLoading: currentAffairsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Subjective Questions
                </Text>
                <NextLink href={`subjective-questions`} passHref legacyBehavior>
                  <Button
